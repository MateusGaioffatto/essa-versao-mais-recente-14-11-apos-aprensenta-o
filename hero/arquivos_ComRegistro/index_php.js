...
user_menu_button.addEventListener('click', function() {
  user_dropdown.style.display = user_dropdown.style.display === 'block' ? 'none' : 'block';
});
...
// ============================================================================
// SISTEMA DE VERIFICAÇÃO DE URL COM VIRUS TOTAL (Implementação completa)
// ============================================================================
...
function verificarUrlSegura(url) {
  ...
  // ATUALIZADO: Usa o virus_total_api.php que está fora da pasta
...
function mostrarResultadoResumidoSeguro(data, url) {
  const cacheBadge = data.cache ? '<span class="badge bg-secondary ms-2">CACHE</span>' : '';
...
// Modal de detalhes
// Modal de detalhes - VERSÃO SIMPLES SEM BACKDROP
function mostrarDetalhesVerificacao(data) {
  ...
  const modalHTML = `
    <div class="modal" id="detalhesVerificacaoModal" tabindex="-1" style="display: block; background-color: rgba(0,0,0,0.5);">
      <div class="modal-dialog modal-lg modal-dialog-centered" style="max-height: 90vh; margin-top: 5vh;">
  ...
  document.getElementById('detalhesVerificacaoModal').addEventListener('click', function(e) {
    if (e.target === this) {
      fecharModalDetalhes();
    }
  });
...
function fecharModalDetalhes() {
  const modal = document.getElementById('detalhesVerificacaoModal');
  if (modal) {
    modal.remove();
  }
}
...
function criarSecaoCache(data) {
  if (!data.cache) return '';
  
  return `
    <div class="alert alert-secondary">
      <strong>💾 Resultado em Cache</strong><br>
      <small>Esta verificação foi recuperada do cache local para otimização de performance.</small>
    </div>
  `;
}
...
document.getElementById('searchButtonId').addEventListener('click', function() {
  const input = document.getElementById('searchInputId').value.trim();
  if (input) {
    // Verificar se é uma URL válida
    if (isValidUrl(input)) {
      verificarUrlSegura(input);
    } else {
      // Se não for URL, redirecionar para página de resultados de produtos
      window.location.href = `resultadosProdutos.php?query=${encodeURIComponent(input)}`;
    }
  } else {
    alert('Por favor, digite uma URL ou termo de busca.');
  }
});
...
function isValidUrl(string) {
  try {
    // Tentar criar uma URL - se funcionar, é uma URL válida
    new URL(string);
    return true;
  } catch (_) {
    // Se não for uma URL completa, verificar se começa com padrões de URL
